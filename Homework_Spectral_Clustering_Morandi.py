...
the clusterization is not optimal.
"""
...
y_range = (-10,15)
x_noise = 0.3
X_an,Y_an = make_bars(n,x_pos,y_range,x_noise,random_state) # Generating bars.
...
"""
In this section we apply the spectral clustering algorithm to the previously defined datasets and show that the
clusterization has been improved.
"""
...
"""
In the first part of this section, we compute different adjacency matrix as well as the eigenvalues and eigenvector for
the corresponding graph Laplacian. Then in the second part we solve two different problems. The first one consist in 
verifying that the discrete sine and cosine functions are eigenvectors of the graph Laplacian associated to the n-cycle.
The second one consists in finding the spectrum of the graph Laplacian associated to the n-hypercube.
"""
